...
LANGUAGES = {
    ...
    "elm":          ("elm",    "elm make Fizzbuzz.elm --output=$T/elm.html >/dev/null && echo 'elm: build only'"),
    ...
    "j":            ("jconsole", "jconsole fizzbuzz.ijs"),
    "k":            ("k",      "k fizzbuzz.k"),
    "q":            ("q",      "q fizzbuzz.q"),
    "futhark":      ("futhark", "futhark c -o $T/fizzbuzz fizzbuzz.fut && echo 1 | $T/fizzbuzz"),

    # ---- query / declarative ------------------------------------------------
    "sql":          ("sqlite3", "sqlite3 -noheader -batch < fizzbuzz.sql"),
    "plpgsql":      ("psql",   "psql -q -f fizzbuzz.sql 2>&1 | sed -e 's/^NOTICE:  //'"),
    "xslt":         ("xsltproc", "xsltproc fizzbuzz.xsl input.xml"),
    "prolog":       ("swipl",  "swipl -q fizzbuzz.pl"),
    "datalog":      ("souffle", "souffle -D- fizzbuzz.dl"),
    "mercury":      ("mmc",    "mmc --output-dir $T -o $T/fizzbuzz fizzbuzz.m && $T/fizzbuzz"),
    "clingo":       ("clingo", "clingo --outf=0 -V0 fizzbuzz.lp"),

    # ---- markup / config ----------------------------------------------------
    "html-css":     ("",       "echo 'html-css: open fizzbuzz.html in a browser (pure CSS counters)'"),
    "svg":          ("",       "echo 'svg: open fizzbuzz.svg in a browser'"),
    ...
    "cue":          ("cue",    "cue eval -e result fizzbuzz.cue"),
    ...
    "rpg":          ("",       "echo 'rpg: requires IBM i (ILE RPG compiler)'"),
    "abap":         ("",       "echo 'abap: requires an SAP NetWeaver system'"),
    ...
    "asm-mips":     ("spim",   "spim -file fizzbuzz.s"),
    "asm-6502":     ("",       "echo 'asm-6502: assemble with ca65/acme and run in a C64 emulator'"),
    "asm-z80":      ("",       "echo 'asm-z80: assemble with z80asm and run under a CP/M emulator'"),
    "wasm-wat":     ("wat2wasm", "wat2wasm fizzbuzz.wat -o $T/fizzbuzz.wasm && wasmtime $T/fizzbuzz.wasm"),
    ...
    "intercal":     ("ick",    "ick -b fizzbuzz.i && ./fizzbuzz"),
    "malbolge":     ("",       "echo 'malbolge: not implemented, see NOTES.md'"),
    "piet":         ("",       "echo 'piet: not implemented, see NOTES.md'"),
    "lolcode":      ("lci",    "lci fizzbuzz.lol"),
    "chef":         ("",       "echo 'chef: partial, see NOTES.md'"),
    "shakespeare":  ("spl2c",  "spl2c < fizzbuzz.spl > $T/fb.c && cc -o $T/fizzbuzz $T/fb.c -lspl && $T/fizzbuzz"),
    "arnoldc":      ("java",   "java -jar $ARNOLDC_JAR fizzbuzz.arnoldc"),
    ...
    "solidity":     ("solc",   "solc --bin FizzBuzz.sol >/dev/null && echo 'solidity: compiled (run all() on-chain)'"),
    "move":         ("move",   "move build"),
    "glsl":         ("glslangValidator", "glslangValidator fizzbuzz.frag && echo 'glsl: compiled (render to see output)'"),
    "hlsl":         ("dxc",    "dxc -T cs_6_0 -E CSMain fizzbuzz.hlsl >/dev/null && echo 'hlsl: compiled (dispatch to see output)'"),
    ...
    "factor":       ("factor", "factor fizzbuzz.factor"),
